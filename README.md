...
| Effect of raising steps | only makes it smoother / more coherent | only makes it smoother / more coherent |
...
- **Step count only moves within the basin**: 5 vs 70 steps at the same canvas changes smoothness/coherence but not the style identity. The canvas decides *which* song you get; steps decide *how polished* that song is.
...
**Practical takeaway**:
- Want a *specific style* → lock 32×32, tune smoothness with steps.
